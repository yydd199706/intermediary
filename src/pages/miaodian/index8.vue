    ...
    <scroll-view class="bd_neirong" scroll-y :scroll-top='scrollTop' :scroll-into-view='toView' style="height:100vh">
      ...
      <!-- 基础信息 -->
        <div class="basic" :id="i.id" >
          <h1>{{i.name}}</h1>
          <div class="b_js">
            <div v-for="(ii,ind) in i.sxxinxi" :key="ind"><span>{{ii.sxbm}}</span>{{ii.name}}</div>            
          </div>
        </div>    
...
export default {
  data () {
     return {
      toView: '',
      scrollTop: 0,
      letters: [{id: 'A',name: '基础信息',sxxinxi: [{sxbm: "楼盘别名",name:"兴安府"},{sxbm: "楼盘特色",name:"限竞房、视频看房、小户型",}]}, 
      {id: 'B',name: '销售信息',sxxinxi: [{sxbm: "售卖状态",name:"在售",}]}, 
      {id: 'C',name: '小区概况',sxxinxi: [{sxbm: "占地面积",name:"43000",}]}, 
      {id: 'D',name: '预售许可证',sxxinxi: [{sxbm: "预售证",name:"京房售证字（2020）10号",}]}
      ],
